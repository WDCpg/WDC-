...
router.post('/saveCalendar', function(req, res, next){
    req.db.getConnection(function(error, connection){
        ...
        connection.query(query,[ req.cookies.session_id, req.body.start, req.body.end, req.body.title, req.body.content, req.body.class],function(error,rows,fields){
            connection.release();
            if(error){
                console.log(error);
                res.sendStatus(500);
...
router.post('/friendAvailability', function(req,res,next){
    req.db.getConnection(function(error, connection){
        if (error){
            console.log(error);
            res.sendStatus(500);
